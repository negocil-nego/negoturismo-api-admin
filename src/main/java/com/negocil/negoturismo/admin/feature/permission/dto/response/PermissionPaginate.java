...
@NoArgsConstructor
@Schema(description = "Paginated response containing a list of permissions")
public class PermissionPaginate extends PaginateResponse<PermissionResponse> {

    public PermissionPaginate(
    ...
    public static PermissionPaginate of(Page<Permission> page) {
        return new PermissionPaginate(
                page.getContent().stream().map(PermissionResponse::of).toList(),
                page.isEmpty(),
                page.isFirst(),
                page.isLast(),
                page.getNumber(),
